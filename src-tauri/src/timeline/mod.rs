...
//! (split/trim/move/delete/duplicate/snap — `ops`), command-based undo/redo
//! (`command`), copy/paste (`clipboard`), the live in-memory session Tauri
//! commands operate on (`session`), and this subsystem's error type
//! (`error`). Operates on the `ProjectV1` schema (`crate::project`) — NOT
//! autocut's single-shared-CutList model (`docs/architecture-audit.md`
//! §4/§5).
//!
