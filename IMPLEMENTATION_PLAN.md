...
- [ ] Implement `src-tauri/src/media/` — ffprobe wrapper, reimplemented from autocut's `probe.rs` design (direct `ffprobe -show_streams -show_format` JSON parsing, chosen over capcut-mate's `pymediainfo` dependency — audit §4): `MediaInfo` struct with duration/fps/width/height/codec/bitrate/audio channels/sample rate/rotation/creation timestamp per master-prompt §7
- [ ] Implement FFmpeg command-builder abstraction (`src-tauri/src/ffmpeg/`) — process argument arrays, never string concatenation (master-prompt §66/§88)
- [ ] Implement FFmpeg/FFprobe sidecar binary resolution (`binaries.rs` pattern from audit §2/§3: layered path search, `#[cfg]` target-triple matrix for Windows x64 MSVC+GNU) and make an independent, documented decision on binary provenance/checksum verification (audit §6 risk #7 — do not inherit autocut's unverified-macOS/manually-rotated-BtbN-pin approach wholesale; document source/version/license per master-prompt §59)
- [ ] Implement audio extraction (`src-tauri/src/audio/`) — PCM extraction via ffmpeg pipe, reimplemented from autocut's `audio.rs` design (incremental i16 read to bound memory), rewritten to the project's i64-µs timebase where relevant
- [ ] Implement waveform generation (`src-tauri/src/audio/waveform.rs`) — peak-per-bin downsampler, reimplemented from autocut's `waveform.rs` design
- [ ] Implement thumbnail generation for video/image import
- [ ] Implement media import: drag & drop, multi-file, folder import, file picker, supported formats per master-prompt §7 (MP4/MOV/MKV/AVI/WEBM/M4V, MP3/WAV/AAC/M4A/FLAC, PNG/JPG/JPEG/WEBP)
- [ ] Implement proxy media generation (master-prompt §8): 4K→720p editing proxy, Off/Auto/Always modes, progress reporting
- [ ] Implement video preview panel: play/pause/stop/seek/frame-step/speed control/canvas ratios (master-prompt §9)
- [ ] Implement local media library indexing (SQLite, `src-tauri/src/db/`) per master-prompt §35
- [ ] Windows path handling tests: spaces, Unicode, Vietnamese filenames (master-prompt §88), reusing capcut-mate's cross-platform test intent (`tests/test_cross_platform.py`) as a reference case list
